...
# Bounding box: désigne les coordonnées du cadre contenant/entourant nos objets spaciaux
# Projected CRS: désigne le système de projection à travers son code EPSG
```
...
#L'informationimportanteestlefaitquenotreobjetsoitunobjetsfETundataframe
```
...
```{r}
st_geometry(communes_Bretagne, lwd= 0.5)
### Carte vide 
```
...
str(communes_Bretagne$surf2) # en m2
```
...
```{r}
# Cela revient à diviser notre variable par 1000*1000. Une façon plus élégante est
# d'utiliser le convertisseur d'unités issu du package units : set_units()
communes_Bretagne <- communes_Bretagne %>% 
...
#- On trouve des fonds "simplifiés" des communes et des fonds détaillés (en général
# de la BDTOPO). Les fonds détaillés sont + précis et donc beaucoup plus lourds.
```
...
# c'est la méthode à préconiser pour regrouper des géométries- rien n'interdit de faire
# un summarise mais on n'a pas toujours une variable numérique à disposition.
```
...
class(centroid_dept_bret$geometry) # type points
```
...
## prend ce qui a de commun
typeof(commune_centroid_bret)

# Il s'agit d'une liste de longueur égale aunombre d'éléments demon 1er paramètre
# (ici cela correspond à mon nombre de communes).
# La fonction intersects renvoie une  liste contenant autant d'éléments que d'observations
# dans notre table communale. Lorsque qu'il y a intersection l'élémen test renseigné, vide sinon.
# On récupère les éléments non vides de notre liste avec la commande suivante
which(lengths(commune_centroid_bret)>0)


#Pouravoirlescommunesconcernées,onappliquecevecteurdepositionànotrefichierdecommunes.
...
# REMARQUE, il est ici plus simple de faire l'intersection dans l'autre sens,
# en mettant les centroid en 1er paramètre

commune_centroid_bret2 <-st_intersects(centroid_dept_bret,communes_Bretagne)
# Cela renvoie une liste de 4 éléments puis de faire
...
# La fonction st_intersection nous renvoit directement le résultat attendu (dataframe).
# Cette fonction est donc beaucoup plus pratique. Cependant, dans le cas de calculs lourds,
# elle peut être très longue.
#
# Avec la fonction within, on regarde si un objet (1er paramètre) est contenu dans un
# autre objet (2e paramètre). L'ordre des paramètre a donc une importance. Cette fonction renvoie encore une liste.
...
communes_Bretagne_wgs84<-communes_Bretagne_wgs84%>%
  mutate(surf3=st_area(geom)) # calcul de l'air 
st_crs(communes_Bretagne_wgs84)

#L'unitéestendegré,lecalculdesdistances,airesnepeutsefairedirectement
# surd'anciennesversionsdesf.
#avecsf1.0.6:lecalculsefaitetlesvaleurssontprochesdesurf2sansêtreégales
```
